...
- **Migrations are SQLite-typed:** `dotnet ef database update` against a fresh SQL Server/Azure SQL DB fails (Guid→String cast in seed `InsertData`; on EF 9+ first as `PendingModelChangesWarning`). Pre-existing since `636655c`, not caused by .NET 10 — prod migrations are manual and prod schema predates it. Don't suppress the warning; see Decision 89 follow-up
