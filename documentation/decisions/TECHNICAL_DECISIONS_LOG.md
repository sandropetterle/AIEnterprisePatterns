...
4. **Purge history.** `git filter-repo` removed `backups/cms/*/dump.sql` and `uploads.tar.gz` from every commit. The first commit carried a GitHub signature that filter-repo cannot keep, so all 251 commits have new SHAs, not just those since 2026-04-09. The force-push required temporarily allowing force pushes on `main`; protection was restored at once. Afterwards no revision contains the dumps or the secret value. 135 PR refs (`refs/pull/*`, which only GitHub can rewrite) still point at old commits, so the dumps stay reachable through old PR commit views and direct old-SHA URLs (e.g. `/commit/3c24b4bd…`, checked 2026-09-24). **GitHub Support was deliberately not asked to clean these up** (see step 5). The 16 SHAs quoted in docs were remapped using filter-repo's `commit-map`.
5. **Accept the residual PR-ref exposure.** GitHub's support portal had no request type for dereferencing PR refs while keeping the PRs. "Repositories → Deletes" is a whole-repo delete/purge form, and the pull-request virtual agent can only delete entire PRs. Deleting 135 PRs (most of the repo's PR history), or deleting and recreating the repo (which also loses issues, Actions secrets and branch protection), costs more than it protects. The only live secret is already rotated, and the remaining hashes belong to a Strapi instance that is local-only and not reachable. A support request is drafted and can still be filed if this changes. It asks Support to dereference `refs/pull/*/head` for PRs 5 6 7 9 11 14 19 23-67 78-143 145-161, then garbage-collect.
...
- (−) All 251 commits have new SHAs. Old SHAs in PR pages, external links and forks no longer match `main`.
- (−) The dumps remain reachable on GitHub through the 135 old PR refs until GitHub garbage-collects them, which it won't do while those refs exist (step 5).
...
- **Verified with a real sign-in (2026-09-23), and the audience risk was real.** The first real create-pattern attempts failed with `401` and `IDX10214: Audience validation failed`, both with the existing session and after a fresh sign-in. The API app registration (`AIPatterns-API`) has `requestedAccessTokenVersion: 2`, so its tokens carry `aud = 862a328f-19ea-4d05-b4c3-54d260bea9ec` (its client ID), not `api://aipatterns-api`. Setting `Authentication__ValidAudiences__0` to that GUID fixed it with no code change. After that, create (Editor+) and delete (Admin) both succeeded on prod, with 0 auth failures in the logs. That proves the audience, issuer, signature and the `MapInboundClaims = false` role fix end to end. The GUID is now committed in `appsettings.Production.json` / `appsettings.Development.json` (a public identifier, not a secret), so the fix doesn't live only in a hand-set env var, which is the drift pattern behind #144. A regression test pins it.
- **Auth rejections are invisible in prod logs by default.** JwtBearer logs failures at Information, and production filters `Microsoft.AspNetCore` at Warning. The API has no App Insights request telemetry (drift table), so diagnosing this needed a temporary `Logging__LogLevel__Microsoft.AspNetCore.Authentication=Information` env var, which was removed afterwards (revision `0000050`). The real `aud` value is PII-masked in the IDX message, so it came from reading the app registration in the External ID tenant (`az login --tenant 8607b218-… --allow-no-subscriptions`).
...
- **Outstanding verification (resolved 2026-09-23, see Decision 91):** `Audience` had been proven only against forged tokens. A real sign-in confirmed the risk: v2 tokens carry the API client-ID GUID as `aud`, so it is now also accepted via `ValidAudiences`.
